...
public class DbHelper {
    ...
    public static final String[] USER_COLS = { DbUser.User.USERNAME, DbUser.User.PASSWORD,
            DbUser.User.ISSAVED };
...
//     * @param tableName
    ...
    public String queryPasswordByName(String username) {
        ...
        if (cursor.getCount() > 0) {
            cursor.moveToFirst();
            password = cursor.getString(cursor.getColumnIndex(DbUser.User.PASSWORD));
        }
    ...
    public int queryIsSavedByName(String username) {
        ...
        if (cursor.getCount() > 0) {
            cursor.moveToFirst();
            isSaved = cursor.getInt(cursor.getColumnIndex(DbUser.User.ISSAVED));
        }
        return isSaved;
    }
    ...
    private static class DBOpenHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            db.execSQL("create table " + USER_TABLE_NAME + " (" + DbUser.User._ID
                    ...
                    + DbUser.User.PASSWORD + " text, " + DbUser.User.ISSAVED + " INTEGER)");
        }
